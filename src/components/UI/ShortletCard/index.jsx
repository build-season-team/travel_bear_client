...
const ShortletCard = ({big, image, header, text, amount, rating, loading}) => {
  return (
    <div className={`${classes.card} ${big ? classes.card__big : ''}`}>
        ...
        <div className={`${classes.card__footer_text} ${big ? classes.card__big__footer_text : ''}`}> {!loading ? `N${amount}` : <Skeleton height={'100%'} width={'100%'} />} </div>
            {!loading ? big ? <Button name='Book Now' bigCard  primary /> : <Button name='Book Now'   primary /> : <Skeleton  />}
        </div>
    </div>
  )
}
